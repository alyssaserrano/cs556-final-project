...
// ============= RETURN HOME BLACK SQUARE TRACKING =================
#define EXPECTED_BLACK_SQUARES 3
  ...
  uint16_t linePos;
    ...
    switch(currentMode){
      ...
      case RETURN_HOME:
        ...
        // Check if we've passed the expected number of black squares
        if(blackSquareCount >= EXPECTED_BLACK_SQUARES){
          Serial.println("Expected black squares reached!  Going to DOCK_ALIGN");
          currentMode = DOCK_ALIGN;
          break;
        }
        ...
        if (returnFlag > BLACK_THRESHOLD){
          ...
          // Skip over the black square so we don't count it multiple times
